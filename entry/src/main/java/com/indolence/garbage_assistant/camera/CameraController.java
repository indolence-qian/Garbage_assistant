...
package com.indolence.garbage_assistant.camera;


import com.indolence.garbage_assistant.CodecAbility;
import com.indolence.garbage_assistant.camera.api.CameraListener;
import com.indolence.garbage_assistant.camera.api.CameraStateListener;
import com.indolence.garbage_assistant.camera.constant.CameraConst;
...
import com.indolence.garbage_assistant.camera.utils.Nv21Handler;
import com.indolence.garbage_assistant.media.VideoRecorder;
import com.indolence.garbage_assistant.media.constant.MediaStatus;
    ...
    private class MyImageArrivalListener implements ImageReceiver.IImageArrivalListener {
        ...
        public void onImageArrival(ImageReceiver receiver) {
            ...
            if (captureMode == CaptureMode.PUSH_FLOW) {
                bytes = getBuffer();
                bytes2 = getBuffer();
                component = image.getComponent(ImageFormat.ComponentType.YUV_Y);
               ...
               // srcOpts.formatHint = "image/jpg";
                component.read(bytes2, 0, component.remaining());
                ByteBuffer byteBuffer2= ByteBuffer.wrap(bytes2);

                imageSource = ImageSource.create(byteBuffer2, srcOpts);


                component = image.getComponent(ImageFormat.ComponentType.YUV_U);
                mBuffer = component.getBuffer();
                mBuffer.get(
                        bytes,
                        resoluteX * resoluteY,
            ...
            String ret = CodecAbilitySlice.mTrackingMobile.MindSpore_runnet(pixelMap);
            LogUtil.info(TAG, "the ret : "+ret);
            if (cameraCallback != null) {
